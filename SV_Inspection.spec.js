...
await page.getByLabel('経費負担場所', { exact: true }).click(); //Location Code column
  await page.locator('#s_1_2_41_0_icon').click();
  await page.getByLabel('部門 リストアプレット:OK').click();
  await page.locator('[id="\\31 _s_1_l_Internal_Work_Staff_Name"]').click(); //Staff Name
  ...
  await page.locator('[class="drilldown"]').first().click(); //Job Card Number column
...
//   await page.getByPlaceholder('拠点名', { exact: true }).press('Alt+Enter');
//   await page.pause();
//   await page.getByRole('button', { name: 'ジョブカード フォームアプレット:再編集' }).click();
...
//   await page.locator('#s_1_2_63_0_icon').click();
