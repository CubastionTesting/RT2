...
test('record demo 1', async () => {
  ...
  //  console.log(generateString(4));
    ...
    await SaleStaffPage.getByRole('combobox', { name: 'Product Class Combobox Field' }).fill('HDT');
  ...
  //  await SaleStaffPage.getByRole('row', { name: 'Opportunity Name Link 1-3IRG43 Account Selection Field Combobox Field Multiple Selection Field Requested Delivery Date Date Field In Progress Product Class ' }).getByRole('gridcell', { name: 'Combobox Field' }).nth(3).click();
   ...
   await SaleStaffPage.getByRole('cell', { name: 'RWD Press F2 for Date Field' }).getByLabel('Press F2 for Date Field').click();
   await SaleStaffPage.getByRole('button', { name: 'Now' }).click();
   await SaleStaffPage.getByRole('button', { name: 'Done' }).click();
   await SaleStaffPage.getByRole('link', { name: 'Chassis', exact: true }).click();
   await SaleStaffPage.getByLabel('Chassis List Applet:New').click();
   await SaleStaffPage.getByLabel('Product Name', { exact: true }).click();
   await SaleStaffPage.getByLabel('Product Name', { exact: true }).fill('FP74GGR2XCVA');
   await SaleStaffPage.getByLabel('Product Name', { exact: true }).press('Enter');
   await SaleStaffPage.getByLabel('Pick Product List Applet:Query').click();
   await SaleStaffPage.getByPlaceholder('<Case Sensitive>').fill('FP74GGR2XCVA');
   await SaleStaffPage.getByPlaceholder('<Case Sensitive>').press('Enter');
   await SaleStaffPage.getByLabel('Pick Product List Applet:OK').click();
   await SaleStaffPage.getByLabel('Product Name', { exact: true }).press('Control+s');
   await SaleStaffPage.getByRole('link', { name: 'Line Details' }).click();
   await SaleStaffPage.getByPlaceholder('Sales Price', { exact: true }).click();
   await SaleStaffPage.getByPlaceholder('Sales Price', { exact: true }).fill('15825000');
   ...
   const validApprovers = jsonData.HDT[0]['Branch-limit']['Branch-Sales-Staf'];
     ...
     await SaleStaffPage.getByPlaceholder('Sales Price', { exact: true }).fill('11510400');
     await SaleStaffPage.getByPlaceholder('Sales Price', { exact: true }).press('Control+s');
     await SaleStaffPage.getByRole('button', { name: 'Quote Form Applet:Generate Approvals' }).click();
     await SaleStaffPage.waitForTimeout(4000);
     await SaleStaffPage.getByRole('link', { name: 'Approval History' }).click();
  ...
  //await SaleStaffPage.locator('[name="s_2_1_127_0"]').click();
  await SaleStaffPage.getByPlaceholder('Sales Price', { exact: true }).dblclick();
  await SaleStaffPage.getByPlaceholder('Sales Price', { exact: true }).fill('6510400');
  await SaleStaffPage.getByPlaceholder('Sales Price', { exact: true }).press('Control+s');
  await SaleStaffPage.getByRole('button', { name: 'Quote Form Applet:Generate Approvals' }).click();
  await SaleStaffPage.waitForTimeout(4000);
  await SaleStaffPage.getByRole('link', { name: 'Approval History' }).click();
  ...
  //const NVApproveruser3= [pageApp1,pageApp2,pageApp1,pageApp2,pageApp2]
  ...
  console.log('From Sales-Staff-Others(Management Limit <-700,000)');
  ...
  const validApprovers1 = jsonData.HDT[2]['Managemant-limit']['Branch-Sales-Staf'];
    const NVApproveruser2= [pageApp1,pageApp2,pageApp1,pageApp1,pageApp1]
