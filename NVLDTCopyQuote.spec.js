...
test('record demo 1', async () => {
  ...
  await page38.getByLabel('Product Name', { exact: true }).fill('FBA20B21S002');
  await page38.getByLabel('Product Name', { exact: true }).press('Enter');
  await page38.getByLabel('Pick Product List Applet:Query').click();
  await page38.getByPlaceholder('<Case Sensitive>').fill('FBA20B21S002');
  ...
  await page38.getByPlaceholder('Sales Price', { exact: true }).fill('3000000');
  ...
  //reject quote scenarios
  console.log("From Sales-Staff - LDT (Branch Limit >= -400,000 )");
  ...
  console.log("From Sales-Staf - LDT (Branch Limit >= -400,000 for Reject case)");
  ...
  //copyQuote
  console.log("From Sales-Staf - LDT (Branch Limit < -400,000 for Copy quote)");
  console.log("Expected - Branch-Sales-Mgr,Branch-Head,SCHQ-BizMgmt-SnrMgr,SCHQ-Sales-NV-Mgr,HQ-President");
