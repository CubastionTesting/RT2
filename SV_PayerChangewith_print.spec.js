...
>>>>>>> 859738c3109be59a39c57ca9ef702218f821e9a0
 ...
 //Delivery date/time
  ...
  await page.locator('[class="drilldown"]').click();
  await page.locator('[aria-label="請求先名"]').click(); //Bill To Basic Account Name column
  await page.locator('[aria-label="請求先名"]').fill('?轄ヲr組　?褐ｳ川建設　建設工事共同企業体');
  await page.locator('[aria-label="請求先名"]').press('Control+s')
 // await page.locator('[id="\\31 _s_2_l_Changed_Sales_Price"]').click();

  
  await page.getByRole('button', { name: '受注変更 フォームアプレット:承認依頼' }).click();
  await page.getByRole('button', { name: '受注変更 フォームアプレット:納品書印刷' }).click(); //Generate Approval buton
  if (await validation.isVisible() == true){
